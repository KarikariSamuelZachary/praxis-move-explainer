...
// services/endgame_recommendation.py). It is resolved on the server before
// the page renders (page.tsx), so the card is present and correct on the
// first paint: no skeleton and no placeholder category that later swaps out.
function RecommendedPanel({
  recommendation,
}: {
  recommendation: EndgameRecommendation;
}) {
    ...
    <section className={`${CARD_CLASS} flex flex-col gap-2 p-3 self-end shadow-2xl shadow-black/25`} aria-label="Recommended for you">
      ...
      <div className="flex items-center gap-3">
        <div className="h-20 w-20 shrink-0 overflow-hidden rounded-lg shadow-lg shadow-black/50 ring-1 ring-black/60">
          <MiniBoard
            options={{
              // Only the starter fallback ships no sample FEN; there the
              // board's own start position is the neutral last resort.
