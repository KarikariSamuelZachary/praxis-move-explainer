...
def main():
    ...
    parser.add_argument(
        ...
        help="Model to use for the selected LLM provider",
    )
    parser.add_argument(
        "--llm",
        choices=["gemini", "groq", "mock"],
        default="gemini",
        help="LLM provider to use (default: gemini, use 'mock' for testing without an API)",
    )
    ...
    if args.llm == "gemini":
        ...
        if not api_key:
            ...
            print("Create a .env file with: GROQ_API_KEY=your-key-here", file=sys.stderr)
            print("Or use --llm mock to test without an API", file=sys.stderr)
            sys.exit(1)
    ...
    if args.llm == "mock":
        ...
        explainer = GroqExplainer(api_key=api_key, model=args.model)
        print(f"   Using Groq: {args.model}")
    else:
        explainer = GeminiExplainer(api_key=api_key, model=args.model)
        print(f"   Using Gemini: {args.model}")

    print("Initializing game analyzer...")
